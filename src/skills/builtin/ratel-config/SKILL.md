---
name: ratel-config
description: 配置与排障 Ratel 自身:模型选择、API 密钥状态、索引、记忆、目标回合上限、诊断。用户问「帮我配模型 / 为什么不工作 / 索引怎么开 / 目标回合改成多少」等配置类问题时激活。
activation: auto
tags: [config]
---
...
- **模型/分块/索引/记忆/外观/目标回合上限**:属于可代改项。先说明要改什么、为什么,征得用户同意后调 `update_app_config`(一次可传多个 key)。用户立目标时若要求把默认回合上限改成 N,改 `goalMaxRounds`(整数 1–100)。
