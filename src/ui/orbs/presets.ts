...
/**
 * 上游预设只标定了 20 / 64 两档。行内更小尺寸（12/14）必须落到最近档，
 * 否则 `PRESETS[mode][size]` 为 undefined，访问 `.count` 会抛错并中断整棵 ChatView 更新
 * （消息已渲染、输入框/发送钮卡在旧 DOM）。
 */
export function coerceOrbSize(size: number): OrbSize {
...
export function resolvePreset(state: OrbState, size: number): Resolved {
  const orbSize = coerceOrbSize(size);
  const key = `${state}-${orbSize}`;
  const hit = cache.get(key);
  if (hit) return hit;

  const mode = STATE_TO_MODE[state];
  const preset = PRESETS[mode][orbSize];
