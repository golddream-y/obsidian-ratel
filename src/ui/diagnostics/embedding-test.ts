...
export function renderEmbeddingTest(container: HTMLElement, plugin: RatelVaultPlugin): void {
	...
	searchResult.createDiv({ cls: 'diag-result-empty', text: tNow('diag.embedding.searchHint') });
	...
	pairSection.createEl('label', { cls: 'diag-label', text: tNow('diag.embedding.textA') });
	const pairA = pairSection.createEl('textarea', {
		cls: 'diag-textarea',
		attr: { placeholder: tNow('diag.embedding.textAPh'), rows: '2' },
	});
	pairSection.createEl('label', { cls: 'diag-label', text: tNow('diag.embedding.textB') });
	...
	pairResult.createDiv({ cls: 'diag-result-empty', text: tNow('diag.embedding.similarityHint') });
