...
const chatZh: ChatStrings = {
  ...
  'chat.error.visionUnsupported': '当前模型不支持图片输入 — 请移除图片、更换支持视觉的模型,或在设置中开启「视觉输入」(本地 Ollama 视觉模型无需开启)',
  'settings.chatVision.name': '视觉输入(图片理解)',
  'settings.chatVision.desc': '当前对话端点的模型支持图片时开启(如 OpenRouter 视觉模型);本地 Ollama 端点自动启用,无需打开',
  'chat.thinking': '思考过程…',
...
const slashZh: SlashStrings = {
  ...
  'slash.goal.description': '立目标并立刻开始;/goal 后直接写要完成的事',
...
const goalZh: GoalStrings = {
  ...
  'goal.slash.conflictSteer': '用户想立新目标:「{next}」。当前进行中的目标是「{current}」。不要拒绝、不要只弹提示、也不要擅自排队。先问用户选:1) 放弃当前并开始新目标 2) 暂停当前、新目标排队 3) 继续当前、忽略新目标。用户选定后再用 manage_goal 执行。',
  ...
  'goal.statusBar.pending': '排队 {count}',
  ...
  'goal.strip.pending': '{objective} · 排队中',
  'goal.strip.pendingMany': '{count} 个目标排队中',
  'goal.strip.blocked': '目标受阻: {reason}',
  'goal.chip.continue': '继续推进: {objective}',
  'goal.chip.takeover': '接管并继续: {objective}',
  'goal.chip.singlePending': '继续排队中的目标: {objective}',
  'goal.chip.multiPending': '{count} 个目标排队',
  ...
  'goal.modal.create.queueOnly': '仅排队',
  ...
  'goal.modal.terminal.completedTitle': '目标已完成',
  'goal.modal.terminal.cancelledTitle': '目标已放弃',
  ...
  'goal.settings.status.pending': '排队',
