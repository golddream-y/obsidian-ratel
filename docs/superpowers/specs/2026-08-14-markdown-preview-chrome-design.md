...
5. **打字机不回归**：流式观感仍为文字持续往下增长，不得变成等待整段完成才出现；具体增量渲染机制以后续 [S-CHAT-PERF](2026-08-15-chat-render-performance-design.md) 为准。
...
> **后续约束更新（2026-08-15）：** [S-CHAT-PERF](2026-08-15-chat-render-performance-design.md) 保留“文字持续增长”的体验目标，但因全量 Markdown 重绘会导致长回复卡死，已取代本节关于 rAF 全量重绘及 overlay 底层持续全量重绘的实现约束。后续实施以 S-CHAT-PERF 为准。

S-MD-MERMAID / 2026-06-27 回归：一旦把「文本节点追加」改成「少次大块替换」或阻塞在 mermaid 异步上，打字机就会变成一段一段刷新。

**P-MD-PREVIEW 实施基线（全量重绘约束已由 S-CHAT-PERF 取代）：**
