...
│   ├── screenshots/            # 新建,GIF/截图存放
│   │   └── chat-demo.gif       # (占位,发版前补)
...
- **截图占位**:关键步骤留 `![screenshot](docs/screenshots/xxx.png)` 占位,0.1.0 发版前补
...
README 嵌入录屏 GIF,不放可交互 HTML(GitHub README 与 Obsidian 商店页均过滤 script/iframe)。

### 8.2 存放路径

`docs/screenshots/`(不放在仓库根,避免根目录膨胀)。
...
### 8.5 录制清单(0.1.0 发版前补,本次 spec 只定规则与占位)
...
- [ ] `docs/screenshots/` 目录创建,留 `.gitkeep` 占位
