...
同一页有「允许访问库外」，默认关闭。打开后才能列库外目录、读库外文件、把文件拷进当前库、运行本机命令。这四项默认询问，安全档和自动档都会确认。对话里的配置不能代开这个开关。
...
聊天输入 `@`：按文件名/路径补全库内笔记，发送时只保留 `@相对路径` 字面量（不预读全文）。也可在文件资源管理器右键 Markdown → **添加到 Ratel**。从访达复制的绝对路径可以贴进输入框；要读这个文件，先打开「允许访问库外」。
...
| **记忆与权限** | 目标默认回合上限与「查看目标」、记忆开关与面板、日记约定、库外访问（默认关）、工具权限档位、全部工具权限（含 MCP 工具） |
