...
社区插件的设置多在各自 `data.json` 里，也有落在本机存储或核心配置文件里的开关。没有统一 API。若让模型对着未知 JSON 现编整文件，无法审计、无法回滚语义、也无法开源复用。档案仍只描述插件 `data.json` 的点名键；不在该文件里的宿主开关由执行层白名单写入，不在本文展开。
...
**不讲：** 50 个热门插件清单、主题/CSS、商店外安装、外观与快捷键。宿主点名设置见执行层，不靠档案描述。
