...
### 3. 检测:按心跳分类处置「慢」「死」「赖」(v1.1 修订)

> **修订说明**:初版结论是「硬超时 30s 无条件击杀,有心跳照样杀(上限即上限)」。落地后复盘发现这会**误杀健康长任务**——一个每秒报进度、正常需要 60s 的批量脚本会在 30s 被杀。心跳机制本身已能区分「卡死」(vm 单线程死循环发不出任何消息)与「慢但在动」(持续 `reportProgress`),分类处置比一刀切更合理:**卡死没决策价值,自动杀;慢是正常业务,决策权交回 LLM**。

| 状态 | 判定 | 动作 |
|---|---|---|
| 正常 | 心跳活跃,按时完成 | 返回结果 |
| 慢(有心跳超时) | 到 `skillScriptTimeout`(默认 30s)仍在持续报进度 | **不终止**。工具返回 `still-running` 结果(附已运行时长 + 最近进度),LLM 决定下一步:再等一轮(`continueRun`)或终止(`killRun`)。可多轮续等,每轮一个超时窗口 |
| 死(无心跳超时) | 整个超时窗口内**零心跳**(同步死循环里 vm 发不出消息) | `worker.terminate()` 毫秒级击杀,返回卡死错误 |
| 赖(绝对上限) | 持续报进度但永不结束(累计 10min,硬编码) | 兜底击杀。forever-progress 脚本没有等下去的价值 |
| 软超时警告 | 10s 无任何消息 | UI Notice 提示「脚本运行较久」(不变,只警告) |

- **同步长计算的固有误杀面**:纯 CPU 密集同步代码(无循环间隙打点)与死循环在 vm 单线程内**不可区分**,会被「无心跳超时」杀。接受——超时窗口可调(skillScriptTimeout),且 skill 脚本模板要求长任务必须打点
- **`skillScriptTimeout` 语义变更**:从「硬超时」改为「无心跳超时窗口」(卡死判定窗口);有心跳时它只决定多久向 LLM 汇报一次进度
- **续等期间来新 run 请求**:隐含终止当前脚本再跑新的(串行链语义,LLM 放弃当前任务的合理推断)

### 4. 异常终止处理:击杀 → 清理 → 错误上抛

```
无心跳超时 / 绝对上限 / killRun / unload
  → worker.terminate()(死循环也能杀)
  → pending Promise 以工具结果 resolve(非 reject)
  → Worker 一次性,杀完即弃无残留
```

错误作为**工具结果**返回而非抛异常——LLM 看到「脚本被终止」可自行换路(改用别的方法、告知用户),不崩回合。`still-running` 与主动 `killRun` 是**有意决策**,不算异常终止。

### 5. 多次失败:熔断,不自动重试

- **不自动重试**——脚本有写文件副作用,盲目重跑可能重复写入
- LLM 再次调用工具视为「重试」,允许;但**同一脚本连续 3 次异常终止(无心跳超时/绝对上限击杀/崩溃)→ 熔断标记**。`still-running` 与 LLM 主动 `killRun` 不计数(前者未终止,后者是有意决策)
- 熔断后再被调用 → 不执行,返回「已连续失败 3 次,需用户重新授权」+ Notice 提醒用户
...
- (v1.1)健康长任务不再被误杀,LLM 拿到进度后能向用户解释「脚本在跑、跑到哪、要不要等」

**负面:**
- 每次执行 ~50ms Worker 启动开销(对手动触发的秒级脚本无感知)
- `vm` 非安全边界,理论上可逃逸——接受(威胁模型内无此对手)
- Worker 内不能访问 Obsidian API,脚本只能走注入的受限 fs 白名单(功能上有意为之)
- (v1.1)sandbox 状态机复杂化:超时不再终结 run,需维护 pendingRun(deferred + 续等/终止/隐含终止三路),测试面扩大
- (v1.1)同步长计算(不打点)与死循环不可区分,仍在无心跳窗口被杀——接受,文档要求长任务打点
...
**v1.1 落地范围(P-SKILL-2-TIMEOUT):** 超时语义按心跳分类(§3 表格)+ `continueRun` / `killRun` 工具参数 + 绝对上限 10min + `skillScriptTimeout` 设置面板控件(补 P-SKILL-2 漏项)+ 熔断计数口径修正。
