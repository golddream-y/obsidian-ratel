...
- **无外部服务** — 网络调用仅:模型 API(默认)+ MCP Server(opt-in,用户显式配置后才出站,见 ADR-014)+ 生态工具(官方清单与所选插件 GitHub release,仅用户调用生态工具时出站,见 ADR-018)。无后台刷新、无遥测。
...
- **网络调用**:默认仅模型 API(DeepSeek / Claude / Ollama);MCP 为 opt-in 出站(ADR-014);生态为第三条网(ADR-018:官方清单 + GitHub release,仅生态工具触发)。隐私边界必须在 README 中写明。
